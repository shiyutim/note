...
- 3.使用 no-store 指令时，暗示请求或响应中包含机密信息。因此不进行缓存。
  4.keep-alive 持久连接； close 关闭；
  5.Date 表示创建报文的日期和时间。
  6.transfer-Encoding 规定了传输报文主体时采用的编码方式。、
  7.accept 首部字段可以通知服务器，用户代理能够处理的媒体类型及媒体类型的相对优先级。
  8.host 是为了区分单台服务器分配多个域名的虚拟主机。如果相同的 IP 地址下部署运行着多个域名，那么服务器就无法理解究竟是哪个域名对应的请求，那么使用 host 来明确指出请求的主机名。
